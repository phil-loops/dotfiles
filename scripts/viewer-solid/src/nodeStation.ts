...
const leaf = (b: string) => b.split("/").pop() ?? b;
...
export function nextStepOf(input: {
  ...
  parentGhost?: string | null; // this node's parent already landed and is droppable
  drifted?: boolean;
  behind?: number;
  syncable?: boolean;
  prepRoute?: string;
  prepWhy?: string;
}): { kind: "contract" | "contract-parent" | "prep"; label: string; title: string } | null {
  // contractable = droppable NOW (rebase-classify exit 20). A merged PR with a follow-on
  // commit is merged but NOT contractable — /contract refuses it, so offer the forward
  // rebase (push the follow-on) instead of a dead drop.
  if (input.merged && input.contractable) {
    return {
      kind: "contract",
      label: "drop ghost & rewire →",
      title:
  ...
  // A node whose PARENT landed is stranded on a base that is about to disappear: syncing it
  // rebases onto a ghost. The contraction belongs to the parent, but it is THIS page's next
  // step, so the slot offers it here rather than making you find the parent's node first.
  if (input.parentGhost) {
    return {
      kind: "contract-parent",
      label: "drop parent ghost & rewire →",
      title: `${leaf(input.parentGhost)} already merged — drop it, rewire this branch onto main, then sync`,
    };
  }
