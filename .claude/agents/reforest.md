...
- **style.md** — type style only; layering, comments, naming and tests are team-owned in the repo's own `AGENTS.md` — read them there. A comment supplying meaning its next line lacks is a naming bug (extract the predicate, name the constant); rationale belongs in the node's description or PR body, not the diff. Length is not the test.
