...
**PARKED (Phil, 2026-08-17): the workflow-adapter branch does not ship.** The one-line alternative (serialize the existing internal `errorCode` string as-is) delivers the functional value whenever it's wanted; the 28-row rename table buys only naming consistency and internal/public decoupling — and the workflow API is beta-gated, so nobody is regexing prose at scale today. The branch (including a concurrent session's six codes + five route stamps + map-fold refactor, preserved with attribution) waits for workflow-API GA or a real customer need. Registry + the idempotency codes ship; the vocabulary in scope is those 13 strings.
...
**End state (Phil, 2026-08-17): `code` goes required — via code-primary, not code-alongside-status.** The optional field and the `code ? … : …` render guard are migration scaffolding only — they exist because legacy call sites bypass the seams and because statuses like 409 have no honest default (a filler code clients would branch on is worse than absence). "Required" must not mean helpers stating a status and its equivalent code in one literal (the two-spellings redundancy the render-seam design removed); instead the arrow inverts: call sites state only the code, the registry declares each code's status once (`statusByApiErrorCode`, satisfies-checked total — generalizing the workflow subsystem's existing internal design), and the seam projects status from code. `defaultCodeByStatus` is the bridge table and gets deleted. Then declare `code` required in the OpenAPI `Error` schema. The sweep ticket carries all of this as its exit criterion.
